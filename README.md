...
pip install -e .
make test    # 940 tests pass from day 1 (945 total; 5 opt-in skips)
...
- **Safety-first deliberation** -- every round table includes six adversarial safety agents by default: Skeptic, Quality, Evidence, FactChecker, Citation, and Sentinel (semantic injection guard, fails closed). Sentinel is advisory by default; opt-in enforcement (`SENTINEL_ENFORCEMENT_ENABLED` or `RoundTableConfig.sentinel_enforcement`) halts a run with an explicit machine-readable refusal on a HIGH screening verdict, a Sentinel refusal, or a missing Sentinel analysis -- an availability tradeoff stated plainly in the [security model](docs/SECURITY_MODEL.md).
...
- **Cost control** -- provider prompt caching (up to ~90% savings on cached content), per-call token tracking, budget enforcement, plus opt-in tiered model routing (`MODEL_ROUTING_ENABLED`): per-role tier selection with budget-aware downgrades and one cascade step up-tier on final failure.
...
- **Validation gates:** every generated project ships with quick checks, red-team scans, architecture tests, CI, and an 18-check validation pipeline before merge.
