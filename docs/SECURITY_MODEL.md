...
flowchart TD
    ...
    Delib --> L3["Layer 3 inside the deliberation: Sentinel semantic screen - refuses on a missing/errored LLM (recorded as critical finding + dissent vote; advisory by default - opt-in SENTINEL_ENFORCEMENT_ENABLED halts the run on a HIGH verdict or a Sentinel refusal)"]
...
flowchart TD
    ...
    L3 -.->|"no LLM available, or the LLM call errors (budget exhausted, not initialized, transport failure)"| Closed["Sentinel REFUSES (sentinel_unavailable): recorded as a critical finding plus a dissent vote - ADVISORY by default: nothing consumes the refusal to short-circuit the deliberation. A shared-client failure makes all core voters dissent (no consensus); a Sentinel-isolated failure leaves screening advisory. Malformed JSON from a live model degrades to a warning instead. Opt-in enforcement (SENTINEL_ENFORCEMENT_ENABLED, default off) halts the run with an explicit machine-readable refusal on a HIGH screening verdict, a Sentinel refusal, or a missing Sentinel analysis"]
...
| The agents themselves | Misbehaving or compromised agents | Per-agent JWT identity verified at dispatch, per-agent rate limits, scope-filtered task context, suspension lifecycle (behavioral baseline recording and vote-lockstep detection are opt-in, detect-only wirings: `BASELINE_TRACKING_ENABLED` / `COLLUSION_DETECTION_ENABLED`, default off) |
...
|---------|--------------|----------------|
| Identity | Per-agent JWT verified before every deliberation phase dispatch (analyze, challenge, vote); tokens are SHA-256 hashed and only the hash is persisted -- the raw token is issued once and held in memory, and a reloaded agent must rotate credentials before it passes verification. Credential currency is surfaced, not silent: register/rotate responses and agent listings return the token's `expires_at` (rotate before it passes; expiry is enforced at dispatch even under the dev kill-switch), and each identity block at the dispatch gates records a deduped `agent_identity_blocked` integrity flag so silently excluded agents are visible to operators | `agents/identity.py`, `agents/registry.py`, `agents/registry_persistence.py`, `orchestration/dispatch_helpers.py`, `learning/flags.py` |
...
| Behavioral baselines | Refusal rate, confidence, latency, and scope discipline compared against each agent's own history -- valid credentials with anomalous behavior still get flagged. Opt-in wiring provided (default off): `BASELINE_TRACKING_ENABLED=true` records round-table/chat dispatches; the deviation analysis itself remains operator-invoked (stated Non-Claim) | `learning/activity.py` |
| Multi-step patterns | Sequences of individually-benign actions that add up to extraction are detected across a window. Opt-in wiring provided (default off): `SEQUENCE_DETECTION_ENABLED=true` runs the scan on the sampled activity pass, detect-only (stated Non-Claim) | `harness/sequence_detector.py` |
...
| Collusion | Lockstep agreement between agents that should be independent is flagged (e.g. two agents defeating four-eyes review). Opt-in wiring provided (default off): `COLLUSION_DETECTION_ENABLED=true` feeds every round-table vote into the analysis; the vote window is per-process memory (stated Non-Claim) | `learning/collusion.py` |
...
flowchart LR
    ...
    Disp --> Mon["Monitoring: activity thresholds + timing + extraction volume (running); baselines / sequence / collusion detectors (opt-in env toggles, default off)"]
...
- **Fail closed, stated precisely.** When persisted metadata fails validation or an identity token is missing, the system skips or blocks -- those gates are enforcing. When Sentinel has no LLM or its LLM call errors (budget exhausted, client not initialized, transport failure; the client marks these with an explicit `is_error`/`error_type` contract), its refusal is recorded as a critical finding (`premise_valid=False`) plus a dissent vote -- but by default no runtime path consumes that refusal to short-circuit the deliberation, so screening on a Sentinel-isolated failure is advisory: with enough other voters the run can still reach consensus on unscreened input. A shared-client failure makes all core voters dissent, so consensus fails. Malformed JSON from a *live* model deliberately degrades to a warning rather than a refusal, so formatting quirks don't hard-fail deliberations.
- **Sentinel enforcement is opt-in and explicit.** Setting `SENTINEL_ENFORCEMENT_ENABLED=true` (or `RoundTableConfig.sentinel_enforcement=True`, which wins over the env flag) makes the round table halt after Phase 1 with a machine-readable refusal (`status="refused"`, `refusal_source="sentinel"`) on exactly three triggers, all keyed to the core Sentinel only: a `HIGH` screening verdict (`sentinel_high_risk`), a Sentinel refusal (`sentinel_unavailable` for fail-closed screening failures, `sentinel_premise` for model-asserted premise refusals), or no Sentinel analysis at all (`sentinel_missing` -- covering a disabled core roster, a core-agent load failure, a dispatch gate, or a crashed `analyze()`). The enforcement signal is captured before the evidence-enforcement pipeline runs, so pipeline drops cannot erase it, and the core Sentinel is exempted from the dispatch rate-limit gate while enforcement is on (without the exemption, the default 100-calls/hour limiter at three Sentinel calls per run caps a process at roughly 33 runs per hour and then refuses everything). This mode trades availability for screening coverage: an LLM outage means refused runs, not unscreened runs. Refusal reasons are distinguished in metrics (`refused_sentinel*` outcomes) so operators can tell a provider outage (`sentinel_unavailable`) from a configuration fault (`sentinel_missing`). Rollback is unsetting the flag and restarting. Default remains off: detect, never act.
...
- **Audit without content.** The deliberation audit trail is structurally metadata-only (phases, counts, durations, outcomes) so prompt and response content cannot leak through it. Each run stores a reasoning-chain hash making after-the-fact edits detectable. Events also carry the acting user in a queryable `user_id` column -- populated on API-driven deliberations, governance reports, and erasures; library writes and pre-migration rows read `''` (attribution is forward-looking, never backfilled).
...
- Sentinel's fail-closed refusal is advisory by default: it is recorded as a critical finding plus a dissent vote, but nothing consumes it to halt the deliberation unless the opt-in enforcement flag is set (`SENTINEL_ENFORCEMENT_ENABLED`, default off). In advisory mode a rate-limited or crashed Sentinel is silently skipped like any other agent -- only enforcement mode guarantees "no Sentinel verdict means no run". Enforcement decides after Phase 1, so the Phase 0/0.5/1 token spend (and the evidence pipeline over Phase 1 analyses) still happens on refused runs.
- The detection hooks (behavioral baseline recording, collusion detection, sequence monitoring) and model routing are off by default -- each needs its env toggle set. When enabled they are detect-only and bounded: the baseline tracker records only round-table/chat dispatches and its deviation analysis is operator-invoked; the collusion vote window is per-process memory; the sequence scan runs only on the sampled activity pass. The canary check (`STARTUP_CANARY_ENABLED`) is a one-shot startup self-test of the injection-defense machinery, not runtime response scanning; correction-drift analysis remains a library you schedule.
...
- **The generated test suite** ([current counts and coverage](FEATURES.md#test-counts), measured on a default full-profile generation) includes dedicated suites for injection defense (including the detect-only user-message scan), agent identity, tamper evidence, governance, the corrections lifecycle, extraction defense (timing regularity, knowledge-read volume, approval-pair escalation), governance reporting, multi-tenant integrity (cross-tenant 404s on the agent control plane, per-tenant activity attribution, the four-eyes posture), the fail-closed contract (LLM error handling, mid-run suspension gating), the opt-in detection/routing wiring (toggles-off regression, per-hook recording, fire-and-forget, plus post-review regressions: per-tenant collusion/baseline scoping, thread-safe vote recording, sequence-flag dedup, and caller-tenant budget context on the round-table route), and credential currency (expiry visibility, identity-block flags, audit user attribution, memory-write provenance, delegation records, tracing degradation).
