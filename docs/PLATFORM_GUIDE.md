...
keyed or filtered by `auth_scope_key(auth)`, an unambiguous encoded tenant/user
scope. To complete isolation, map
...
|------------|------------------|--------------|
| Chat sessions (`_orchestrators`) | Keyed by encoded auth scope + session ID | **Already isolated** |
| Harness sessions (`_sessions`) | Keyed by encoded auth scope + session ID | **Already isolated** |
| Round table result cache | Keyed by encoded auth scope + task ID | **Already isolated** |
| Transcript search index | Stores and filters by encoded auth scope owner key | **Already isolated** |
...
| Session isolation | **Built** | Encoded tenant/user scope + session ID |
