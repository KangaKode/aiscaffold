...
- **Monitoring** -- health checks on demand, per-dispatch stats (latency, refusals, scope violations), and override screening on knowledge writes. Behavioral-baseline and collusion detectors ship with opt-in wiring behind default-off env toggles (`BASELINE_TRACKING_ENABLED`, `COLLUSION_DETECTION_ENABLED`); see GOVERNANCE.md for what they observe and their limits.
