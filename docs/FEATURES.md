...
- **Capability Doctor** -- Ops health matrix (`aiscaffold doctor`, generated `scripts/capability_doctor.py` / `make doctor`) with states `live` / `broken` / `declined` / `stale` / `unconfigured`. Structure failures still block; capability `broken` warns unless `--strict-capabilities`. Does **not** replace `validate_generated.sh` or CI. Maintainer-checkout pre-commit Bugbot + Security Review receipt gating ships via Cursor `beforeShellExecution` and native `.githooks` (`make hooks-install`); see `docs/DEVELOPMENT_PROCESS.md`.
