...
mutates or filters the tool list. Flags persist via record_flag_hit when
a store and MCPServerConfig are provided.
...
def screen_listed_tools(
    ...
    """Screen tools detect-only; optionally update hashes and record flags.

    ``config`` must expose ``name``, ``tenant_id``, and ``tool_desc_hashes``
    (dict) when hash memory / flags are desired. Without config: log-only.
    """
    flagged = 0
    drifted = 0
    scanned = 0
    ...
    tenant_id = getattr(config, "tenant_id", "default") if config else "default"
...
def _flag(
    ...
    if store is None or not server_name:
        return
    try:
        ...
        record_flag_hit(
            store,
            flag_type=flag_type,
            subject_id=f"{server_name}:{tool_name}",
            tenant_id=tenant_id,
            detail=detail,
            severity="warning",
        )
    except Exception:
        logger.warning("[ToolScreen] Flag persist failed (non-fatal)", exc_info=True)
