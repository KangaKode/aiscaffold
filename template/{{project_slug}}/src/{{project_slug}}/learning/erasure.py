...
Derived artifacts (measured coverage, honest scope):
  - Error schemas (error_schemas) are DERIVED from approved corrections:
    ...
    erased text does not survive in generalized form.
...
def erase_correction(
    ...
    error schemas that cite it (see module docstring for the measured
...
def _erase_derived_schemas(
    ...
    if deleted:
        ...
        try:
            ...
            rebuilt = len(extract_error_schemas(store, tenant_id=tenant_id))
        except Exception as exc:
