...
| Knowledge compaction | Clusters of approved corrections generalize into reusable error schemas (deterministic templating, >= 3 corrections from >= 2 distinct proposers) that feed single-shot resolution context alongside raw corrections | `learning/error_schemata.py`, `orchestration/single_shot.py` (`error_schemas` table) | `tests/test_learning_maturity.py` |
| Contradiction detection | Pairwise TF-IDF similarity + negation heuristic over a tenant's approved corrections flags pairs that likely disagree ("use X" vs "stop using X"); findings persist as integrity flags for human review, never auto-retired | `learning/contradiction.py` | `tests/test_learning_maturity.py` |
