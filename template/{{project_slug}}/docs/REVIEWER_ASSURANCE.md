...
ships with this project. It is the single source of truth that gates
...
fixture land in a later change.
...
- **`BLOCKING`** — reviewer has cleared the promotion protocol against
  ...
  records live alongside this table once the promotion protocol lands.
...
governed by this register — their exit-code semantics stand.
...
- Blocking-evidence contract: `.cursor/rules/expert-review.mdc` —
  Proof of Finding section (six required fields, correctness branch,
  and `UNVERIFIED` label).
