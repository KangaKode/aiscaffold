...
| ASI02 | Tool Misuse and Exploitation | MCP tool output and remote-agent responses are sanitized and full Layer 1-2 scanned (log-only), so untrusted tool text cannot silently steer a downstream prompt undetected; MCP tool **descriptions/schemas** are Layer 1-2 scanned on every `list_tools` fetch (hash drift memory + advisory health counts) — detect-only by default; opt-in `MCP_TOOL_METADATA_ENFORCEMENT_ENABLED` omits/refuses **injection**-flagged tools only (drift stays advisory; never auto-sets `enabled=False`); per-agent rate limits and scope filtering bound what a misused dispatch can reach. | `orchestration/mcp_enrichment.py`, `connectors/tool_screen.py`, `connectors/mcp_client.py`, `agents/remote.py`, `agents/rate_limiter.py`, `orchestration/scope_filter.py` |
...
| ASI04 | Agentic Supply Chain Vulnerabilities | **Partial.** Vendored red-team data carries pinned provenance (immutable upstream commit SHA + per-seed SHA-256 over exact code points), recomputed offline by a shipped test. MCP tool metadata fetched from servers is scanned detect-only for injection/drift (ASI02) — not a provenance or update-channel guarantee. The scaffold does NOT vet your models, MCP servers, or remote agents' trustworthiness at runtime; sandbox/egress containment is not claimed (ASI05). | `tests/fixtures/provenance.json`, `tests/test_adversarial_open_corpus.py`, `connectors/tool_screen.py` |
