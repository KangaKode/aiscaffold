...
in [`DEVELOPMENT_PROCESS.md`](DEVELOPMENT_PROCESS.md) and the
...
| Status | `DRAFT`, `SHADOW`, `BLOCKING`, or `SUSPENDED` — same closed vocabulary as [`REVIEWER_ASSURANCE.md`](REVIEWER_ASSURANCE.md) |
...
- Scoped reviewers (`.cursor/agents/*.md`) follow the same boundary
  defined in [`.cursor/rules/expert-review.mdc`](../.cursor/rules/expert-review.mdc):
  no merge, no fix, no self-edit-of-own-rules, no self-promotion.

## Non-claims

- **CI does not enforce the three-artifact gate.** It is a
  human/review gate. Documentation-parity tests in the scaffold
  assert that this register, the process docs, and the always-applied
  rules reference each other; they do not judge whether a specific
  fix meets the three-artifact bar.
...
- **Deterministic scanners are outside this register.** Their
  ...
  [`.cursor/rules/expert-review.mdc`](../.cursor/rules/expert-review.mdc)
