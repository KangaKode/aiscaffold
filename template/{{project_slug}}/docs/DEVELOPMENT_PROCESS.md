...
may only recommend `BLOCK` when their version is recorded as
`BLOCKING` in
...
`scripts/reviewer_eval.py`, but the `MANUAL_AGENT` cases must be
fed to each reviewer in a fresh context by a human operator (no
authenticated agent runner exists in GitHub Actions).
