...
| `/api/v1/procedures` | POST | Propose a governed procedure (`type=procedure`; `procedure_screen` hard-refuse; same PII/content-policy/override stack as corrections) |
| `/api/v1/procedures` | GET | List this tenant's procedures (shares extraction-guard volume with corrections/reflections) |
| `/api/v1/procedures/{id}/approve` | POST | Approve -- same four-eyes posture as corrections; does **not** auto-ground into prompts |
| `/api/v1/procedures/{id}/supersede` | POST | Propose a same-type successor procedure |
| `/api/v1/procedures/{id}/reject` | POST | Reject a proposed procedure (terminal) |
| `/api/v1/procedures/{id}/retire` | POST | Retire an approved procedure |
| `/api/v1/procedures/{id}/revalidate` | POST | Re-confirm an approved procedure |
| `/api/v1/procedures/{id}` | DELETE | GDPR Art. 17 hard-delete (same erasure rules as corrections) |
...
- **Procedures are a separate resource.** Propose, list, approve, reject, retire, revalidate, supersede, and erase step-shaped knowledge via `/api/v1/procedures` — not via `/api/v1/corrections`. Claim corrections stay on the corrections routes (`type=""`). Approving a procedure does **not** put it into default agent grounding; default `knowledge_context` remains claim-only until an explicit opt-in procedure-aware render path is wired (not default-on in this release).
...
- **Extraction guard**: `learning/extraction_guard.py` counts knowledge-endpoint reads (GET /corrections, GET /reflections, and GET /procedures) per user and tenant-wide over a rolling window (`EXTRACTION_USER_THRESHOLD` / `EXTRACTION_TENANT_THRESHOLD` / `EXTRACTION_WINDOW_MINUTES`) and maps them to normal/elevated/capped. Detection-only by default; the opt-in enforcement point (`EXTRACTION_GUARD_ENFORCE=true`) is a 429 + Retry-After on `GET /corrections` and `GET /procedures` while capped (shared volume -- no pivot bypass).
...
- **Every deployment**: `STARTUP_CANARY_ENABLED=true`. It is a one-shot self-check at boot with no per-request cost, and a failure means the injection-defense machinery itself is broken -- something you want flagged before the first real request, not discovered during an incident. Add `RUNTIME_CANARY_ENABLED=true` when you want live leak signals on final responses; add `RUNTIME_CANARY_ENFORCEMENT_ENABLED=true` only when you are willing to refuse those responses (both default off).
...
- **Direct invocation** (`POST /api/v1/mcp/servers/{name}/invoke`): call one tool ad hoc; output is sanitized before it leaves the platform, and errors come back as structured fields, never stack traces. Disabled servers return the same 404 as missing (no disabled oracle).
- **Opt-in metadata enforcement** (`MCP_TOOL_METADATA_ENFORCEMENT_ENABLED`, default off): after a successful health/`list_tools` screen, injection-flagged tools are omitted from listings and `call_tool` returns stable `error_code=tool_refused_metadata_injection` before transport. Drift stays detect-only; `enabled` is never auto-mutated. See `docs/GOVERNANCE.md` Non-Claims.
...
| Runtime canary (detect / refuse) | **Opt-in wiring provided (default off)** | `RUNTIME_CANARY_ENABLED=true` injects+checks on chat synthesis, `/resolve`, and round-table synthesis (flags `canary_leak`). `RUNTIME_CANARY_ENFORCEMENT_ENABLED=true` refuses on leak (no-op unless detection is on). Not always-on; not ASI05/ASI10 |
