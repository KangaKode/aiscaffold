...
keyed or filtered by structured `(tenant_id, user_id)` scope keys. To complete isolation, map
...
|------------|------------------|--------------|
| Chat sessions (`_orchestrators`) | Keyed by `(tenant_id, user_id, session_id)` tuple | **Already isolated** |
| Harness sessions (`_sessions`) | Keyed by `(tenant_id, user_id, session_id)` tuple | **Already isolated** |
| Round table result cache | Keyed by `(tenant_id, user_id, task_id)` tuple | **Already isolated** |
| Transcript search index | Stores and filters by encoded `(tenant_id, user_id)` owner key | **Already isolated** |
...
| Session isolation | **Built** | Structured `(tenant_id, user_id, session_id)` cache keys |
